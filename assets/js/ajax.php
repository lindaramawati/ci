<script type="text/javascript">
	...
	function tampilInvoice() {
		$.get('<?php echo base_url('admin/Invoice/tampil'); ?>', function(data) {
			MyTable.fnDestroy();
	...
	var id_invoice;
	$(document).on("click", ".konfirmasiHapus-invoice", function() {
		id_invoice = $(this).attr("data-id");
	})
	$(document).on("click", ".hapus-dataInvoice", function() {
		var id = id_invoice;
	...
	$(document).on("click", ".update-dataInvoice", function() {
		var id = $(this).attr("data-id");
		
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('admin/Invoice/update'); ?>",
			data: "id=" +id
		})
		.done(function(data) {
	...
	$('#form-tambah-invoice').submit(function(e) {
		...
		$.ajax({
			method: 'POST',
			url: '<?php echo base_url('admin/Invoice/prosesTambah'); ?>',
			data: data
		})
		.done(function(data) {
			...
			if (out.status == 'form') {
				...
				document.getElementById("form-tambah-invoice").reset();
				$('#tambah-invoice').modal('hide');
				$('.msg').html(out.msg);
				effect_msg();
			}
		})
	...
	$(document).on('submit', '#form-update-invoice', function(e){
		...
		$.ajax({
			method: 'POST',
			url: '<?php echo base_url('admin/Invoice/prosesUpdate'); ?>',
			data: data
		})
		.done(function(data) {
			var out = jQuery.parseJSON(data);
			...
			if (out.status == 'form') {
				...
				document.getElementById("form-update-invoice").reset();
				$('#update-invoice').modal('hide');
				$('.msg').html(out.msg);
				effect_msg();
			}
		})
	...
	$('#tambah-invoice').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})

	$('#update-invoice').on('hidden.bs.modal', function () {
	  $('.form-msg').html('');
	})
